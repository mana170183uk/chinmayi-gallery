...
import { artworks, collections, testimonials } from "@/data/artworks";
...
export default function HomePage() {
  const featured = artworks.find((a) => a.slug === "twilight-reverie")!;
  const newArrivals = artworks.filter((a) => a.badge === "new").slice(0, 4);
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center max-w-[1400px] mx-auto w-full relative z-[3]">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.3 }} className="grid grid-cols-2 gap-4" style={{ perspective: "800px" }}>
            {artworks.slice(0, 4).map((art, i) => (
              ...
              >
                <div className="art-gradient" style={{ background: art.gradient, aspectRatio: art.aspectRatio }} />
            ...
            >
              <Link href={`/gallery?collection=${col.id}`}>
                <div className="rounded-2xl overflow-hidden relative border group cursor-pointer" style={{ aspectRatio: "3/4", borderColor: "var(--border)" }}>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-8 transition-all group-hover:from-black/85">
      ...
      <section className="py-24 px-6 md:px-14 relative z-[1]" style={{ background: "var(--bg2)" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center max-w-[1200px] mx-auto relative z-[1]">
          <div className="rounded-2xl overflow-hidden relative border" style={{ boxShadow: "var(--art-glow), var(--art-shadow)", borderColor: "var(--border2)" }}>
            <div className="art-gradient" style={{ background: featured.gradient, aspectRatio: "3/4" }} />
          </div>
          <div>
            ...
            <div className="font-[Playfair_Display] text-[34px] font-bold mb-7" style={{ color: "var(--gold)" }}>
              ...
              {featured.originalPrice && (
                <span className="text-[18px] line-through ml-3 font-normal" style={{ color: "var(--text3)" }}>£{featured.originalPrice.toLocaleString()}</span>
              )}
            </div>
            <div className="flex gap-3 items-center flex-wrap">
              ...
              <button className="w-[52px] h-[52px] rounded-lg inline-flex items-center justify-center border text-[20px] transition-all hover:border-[var(--rose)] hover:text-[var(--rose)]" style={{ borderColor: "var(--border)", color: "var(--text2)" }}>
                ♡
              </button>
            </div>
          </div>
      ...
      <section className="py-24 px-6 md:px-14 relative z-[1]">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-16">
          ...
          <motion.h2 variants={fadeUp} className="text-[clamp(30px,4vw,50px)] font-semibold">About Chinmayi</motion.h2>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-16 items-center max-w-[1200px] mx-auto">
          ...
          <div>
            ...
            <p className="text-[15px] leading-relaxed mb-4" style={{ color: "var(--text2)" }}>
              Chinmayi is a contemporary fine artist whose work explores the intersection of emotion, colour and texture. Drawing inspiration from natural landscapes, human connections and the quiet moments in between.
            </p>
            <p className="text-[15px] leading-relaxed mb-4" style={{ color: "var(--text2)" }}>
      ...
      <section className="py-24 px-6 md:px-14 relative z-[1]" style={{ background: "var(--bg2)" }}>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-16">
          ...
          <motion.p variants={fadeUp} className="text-[16px] max-w-[560px] mx-auto" style={{ color: "var(--text2)" }}>
            Hear from art lovers who have welcomed Chinmayi&apos;s work into their homes
          </motion.p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[1200px] mx-auto">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ delay: i * 0.15, duration: 0.6 }}
              className="p-8 rounded-2xl backdrop-blur-xl border transition-all hover:border-[var(--card-hover-border)] hover:-translate-y-1"
      ...
      <section className="py-24 px-6 md:px-14 relative z-[1]" style={{ background: "var(--bg2)" }}>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-8">
          ...
          <motion.h2 variants={fadeUp} className="text-[clamp(30px,4vw,50px)] font-semibold">Join the Inner Circle</motion.h2>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-[560px] mx-auto text-center">
          ...
          <form
            ...
            onSubmit={(e) => { e.preventDefault(); alert("Welcome to the Chinmayi Gallery family!"); }}
          >
            <input
              type="email"
            ...
            />
            <button type="submit" className="px-8 py-3.5 rounded-md text-[13px] font-semibold tracking-wider uppercase transition-all hover:-translate-y-0.5" style={{ background: "linear-gradient(135deg, var(--gold), var(--gold2))", color: "#1A1830" }}>
              Subscribe
            </button>
          </form>
        </motion.div>
