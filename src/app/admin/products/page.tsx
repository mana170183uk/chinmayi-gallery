...
interface Product {
  id: string; title: string; type: string; description: string; price: number;
...
export default function AdminProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const body = { ...form, price: form.price, id: editId };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button onClick={() => { setShowForm(true); setEditId(null); setForm(emptyForm); }} className="px-6 py-2.5 rounded-lg text-[13px] font-semibold tracking-wide transition-all hover:shadow-lg" style={{ background: "linear-gradient(135deg, var(--gold), var(--gold2))", color: "#1A1830" }}>+ Add Product</button>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] bg-black/60 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border p-8" style={{ background: "var(--bg)", borderColor: "var(--border)" }} onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {form.type === "clothing" && (
                  <div>
                    <label className="block text-[12px] uppercase tracking-wider font-semibold mb-2" style={{ color: "var(--text3)" }}>Sizes</label>
                ...
                {/* Image Upload */}
                <div>
                  <label className="block text-[12px] uppercase tracking-wider font-semibold mb-2" style={{ color: "var(--text3)" }}>Product Image</label>
            ...
            {filtered.map(p => (
              <tr key={p.id} className="border-b hover:bg-[var(--bg2)]" style={{ borderColor: "var(--border)" }}>
                ...
                <td className="px-6 py-3 text-right">
                  <button onClick={() => { setForm({ title: p.title, type: p.type, description: p.description, price: String(p.price), material: p.material || "", sizes: p.sizes || "", imageUrl: p.imageUrl || "", badge: p.badge || "", gradient: p.gradient }); setEditId(p.id); setShowForm(true); }} className="px-3 py-1 text-[12px] mr-2 hover:text-[var(--gold)]" style={{ color: "var(--text2)" }}>Edit</button>
                  <button onClick={() => handleDelete(p.id)} className="px-3 py-1 text-[12px] hover:text-[var(--rose)]" style={{ color: "var(--text3)" }}>Delete</button>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center" style={{ color: "var(--text3)" }}>No products yet. Click &quot;+ Add Product&quot; to get started.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
