    ...
    <section className="min-h-screen pt-36 pb-24 px-6 md:px-14 relative z-[1]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-[1200px] mx-auto">
        {collections.map((col, i) => (
          <motion.div key={col.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, duration: 0.6 }}>
            <Link href={`/gallery?collection=${col.slug || col.id}`}>
              <div className="rounded-2xl overflow-hidden relative border group cursor-pointer transition-all hover:-translate-y-2 hover:shadow-xl" style={{ aspectRatio: "3/4", borderColor: "var(--border)" }}>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-8">
                  ...
                  <span className="inline-flex items-center gap-2 text-[12px] font-semibold tracking-wider uppercase text-white/80 group-hover:text-[var(--gold)] transition-colors">View Collection →</span>
                </div>
              </div>
            </Link>
          </motion.div>
